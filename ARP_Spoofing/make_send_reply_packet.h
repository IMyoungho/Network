...
void send_reply_arp(pcap_t* pcd, uint8_t packet[42],int packet_len)
{
    ...
    while(true)
    {
        pcap_sendpacket(pcd,(u_char*)packet,packet_len);
        sleep(2);
...
void make_send_reply_packet(parse_data *parse)
{
    ...
    memcpy(reply_packet,parse->using_target_mac(),6);
    ...
    memcpy(reply_packet+28,parse->using_gateway_ip(),4);
    memcpy(reply_packet+32,parse->using_target_mac(),6);
    memcpy(reply_packet+38,parse->using_target_ip(),4);
    pcap_t *pcd;
