...
void parse_data::check_argc(int argc, char *argv[])
{
    if(argc!=4)
    {
        ...
        cout << " >> < Usage > : < interface > < Gateway ip > <Target ip> " << endl;
        exit(1);
    }
    inet_pton(AF_INET, argv[2],&this->gateway_ip);
    inet_pton(AF_INET, argv[3],&this->target_ip);
}
...
void parse_data::get_gateway_mac(uint8_t mac[6]){
    memcpy(this->gateway_mac,mac,6);
}
